...
async function main() {
  ...
  const hysaNewHouseInstitution = await ask('Institution for HYSA New House', '');
  ...
  const schoolLoanStartDate = await ask('School loan start date (YYYY-MM-DD)', '2015-10-01');
  const brokerageAccountName = await ask('Taxable brokerage account name', 'Brokerage');
  const brokerageInstitution  = await ask('Taxable brokerage institution', 'Robinhood');

  const collapseSchoolLoans = await ask('Collapse 8 school loan tranches into one loan? (y/n)', 'n');
  ...
  const household = await prisma.household.create({ data: { name: householdName, pinHash } });

  // ── Members ────────────────────────────────────────────────────────────────
  const allan = await prisma.member.create({
    data: { householdId: household.id, name: 'Allan', email: allanEmail, color: '#3B82F6' },
  });
  const malia = await prisma.member.create({
    data: { householdId: household.id, name: 'Malia', email: maliaEmail, color: '#EC4899' },
  });
  ...
  for (const cat of EXPENSE_CATEGORIES) {
    const created = await prisma.expenseCategory.create({
      data: { householdId: household.id, name: cat.name, icon: cat.icon, color: cat.color, sortOrder: cat.sortOrder },
    });
  ...
  for (const [name, color] of Object.entries(invCatColors)) {
    await prisma.investmentCategory.create({
      data: { householdId: household.id, name, color, sortOrder: invCatOrder++ },
    });
  }
  ...
  if (singleLoan) {
    ...
    const avgRate = SCHOOL_LOANS.reduce((s, l) => s + l.rate, 0) / SCHOOL_LOANS.length;
  ...
  for (const rm of reviewMonths) {
    ...
    // Income entries
    const incomeEntries = MONTHLY_ENTRIES.filter((e) => e.month === rm.month && e.type === 'Income' && e.amount > 0);
    for (const entry of incomeEntries) {
      let memberId: number | null = null;
      for (const [prefix, member] of Object.entries(memberByPrefix)) {
        if (entry.title.startsWith(prefix)) { memberId = member.id; break; }
      }
      const name = entry.title.replace(/^(Allan|Malia): /, '');
      await prisma.incomeEntry.create({
        data: { reviewId: review.id, memberId: memberId ?? allan.id, name, notes: entry.notes ?? null, amount: cents(entry.amount) },
      });
    }

    // Expense entries
    const expenseEntries = MONTHLY_ENTRIES.filter((e) => e.month === rm.month && e.type === 'Expense' && e.amount > 0);
    for (const entry of expenseEntries) {
      const categoryId = categoryIdByLabel(entry.category);
      if (!categoryId) { console.warn(`    ⚠ Unknown category "${entry.category}" for "${entry.title}", skipping`); continue; }
      let memberId: number | null = null;
      for (const [prefix, member] of Object.entries(memberByPrefix)) {
        if (entry.title.startsWith(prefix)) { memberId = member.id; break; }
      }
    ...
    for (let i = 0; i < SAVINGS_ACCOUNTS.length; i++) {
      ...
      const deposits = acc.inputByMonth[monthKey as keyof typeof acc.inputByMonth];
      const interest = acc.interestByMonth[monthKey as keyof typeof acc.interestByMonth];
    ...
    if (rm.month === 3) {
      ...
      await prisma.loanSnapshot.create({
        data: {
          ...
          paymentAmount: cents(1337.50),
          principalAmount: cents(1337.50 - MORTGAGE.marInterestPaid),
          extraPayment: 0,
        },
      });
      ...
      if (singleLoan && schoolLoanRecords[0]) {
        ...
        await prisma.loanSnapshot.create({
          data: {
            ...
            paymentsMade: 0,
            interestPaid: cents(SCHOOL_LOANS.reduce((s, l) => s + (l.balance * l.rate / 12), 0)),
            extraPayment: 0,
          },
        });
      } else {
    ...
    if (rm.month === 3) {
      ...
      const taxablePurchaseIds: Array<{ id: number; ticker: string; shares: number; pricePerShare: number }> = [];
      for (const lot of TAXABLE_LOTS) {
        ...
        taxablePurchaseIds.push({ id: p.id, ticker: lot.ticker, shares: lot.shares, pricePerShare: p.pricePerShare });
      }

      // Insert retirement purchase lots
      for (const lot of RETIREMENT_LOTS) {
        const accId = retirementAccountMap.get(lot.account);
        if (!accId) { console.warn(`    ⚠ No account found for "${lot.account}", skipping lot ${lot.ticker}`); continue; }
      ...
      for (const p of taxablePurchaseIds) {
        ...
        if (!currentPrice) { console.warn(`    ⚠ No current price for ${p.ticker}, skipping holding snapshot`); continue; }
        ...
        await prisma.holdingSnapshot.create({
          data: { purchaseId: p.id, reviewId: review.id, price: priceInCents, value, gainLoss: value - costBasis },
        });
      }
      ...
      for (const acc of RETIREMENT_ACCOUNTS) {
        ...
        await prisma.retirementSnapshot.create({
          data: { accountId: accId, reviewId: review.id, balance: cents(acc.marBalance) },
        });
      }
      ...
      for (const { id, vault } of vaultRecords) {
        ...
        await prisma.vaultSnapshot.create({ data: { vaultId: id, reviewId: review.id, amount } });
      }
    }

    const incomeCount = incomeEntries.length;
    const expenseCount = expenseEntries.filter((e) => e.amount > 0).length;
    console.log(`  ✓ Review ${rm.month}/2026 — ${incomeCount} income, ${expenseCount} expense entries`);
  }
